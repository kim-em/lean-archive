...
Runs `ArchiveTest.FuzzHandleRead.runFuzzUntil` for a wall-clock budget
...
This is the deterministic regression harness for the
`lean_io_prim_handle_read` bug class (lean4#13388); see
`SECURITY.md` *Local guard inventory for `Handle.read` and
`Stream.read`*. -/
...
deadline={deadline}ms"
  let iterations ← ArchiveTest.FuzzHandleRead.runFuzzUntil seed deadline
