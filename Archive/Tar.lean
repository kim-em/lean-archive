...
/-- Thin bounded-length `readExact`-style helper over `IO.FS.Stream`, adding the
    ...
    Cross-reference: `SECURITY.md` § "Local guard inventory for
...
/-- General-purpose bounded wrapper around `readEntryData` that enforces a
    ...
    **per-call** cap primitive — typically the stricter of the two. Cross-
    reference: `SECURITY.md` § "Local guard inventory for
    `Handle.read` and `Stream.read`". -/
...
/-- Extract a tar archive from input stream to output directory.
    ...
    See `SECURITY.md` *Decompression and extraction limits*.
...
/-- Extract a .tar.gz archive.
    ...
    outer gzip-stream cap on this variant: lean-zlib's streaming gzip
    has no `maxOutputSize` parameter today (a known gap; see
    `SECURITY.md` *Decompression and extraction limits*).
