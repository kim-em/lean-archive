...
/-- maxTotalSize whole-archive bomb regression (SECURITY.md: whole-archive running-sum cap):
    ...
    keep `ArchiveTest.Tar.tests`'s single `do` block under the elaborator's
    recursion limit. -/
...
def ArchiveTest.Tar.tests : IO Unit := do
  ...
  -- the limit must be rejected before any payload is read (Archive/Tar.lean:758-759).
