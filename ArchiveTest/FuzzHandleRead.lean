...
Sibling of lean-zip’s `conformance/Conformance/FuzzInflate.lean`: same xorshift PRNG, same
...
See `SECURITY.md` *Local guard inventory for `Handle.read` and
`Stream.read`* for the per-site audit whose guarantees this
harness exercises. -/

namespace ArchiveTest.FuzzHandleRead
...
/-- 64-bit xorshift PRNG. Inlined here so the fuzz driver has zero
    ...
    Identical to the inlined PRNG in lean-zip’s `conformance/Conformance/FuzzInflate.lean`. -/
...
/-- Try wrapper for an `IO`-returning archive operation. `.userError`
    ...
    so we re-raise. Mirrors `tryFFI` in lean-zip’s `conformance/Conformance/FuzzInflate.lean`. -/
...
/-- Resolve the fuzz scratch directory. Reused across iterations
    ...
    by `ArchiveTest.Zip.tests` and `ArchiveTest.Helpers.writeFixtureTmp`). -/
...
end ArchiveTest.FuzzHandleRead
