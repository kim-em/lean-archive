...
/-! Build malformed UStar header regression fixtures.
...
`gname`). The `Tar.parseHeader` `hasInteriorNul` guard at `Archive/Tar.lean`
...
/-- Fixture: ustar-prefix-nul-in-name.tar
    ...
    [Archive/Tar.lean:389], bypassing the auto-`splitPath` logic.
    ...
    [Archive/Tar.lean:542] would yield `"badpfx/name.txt"` (parser
