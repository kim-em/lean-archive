...
/-! Build malformed PAX regression fixtures.
...
`parsePaxRecords` guards in `Archive/Tar.lean` should silently skip the
...
/-- PAX record `"14 path=a\x00b/c\n"` — a `path` override whose value
    ...
    at [Archive/Tar.lean:145] the override would reach `applyPaxOverrides`
...
/-- PAX record `"18 linkpath=a\x00b/c\n"` — a `linkpath` override whose
    ...
    `valueBytes` at [Archive/Tar.lean:146] the override would reach
    ...
    [Archive/Tar.lean:161-162]. With the guard, the record is silently
